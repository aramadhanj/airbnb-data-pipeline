...
dataset_url = f"http://data.insideairbnb.com/thailand/central-thailand/bangkok/2023-06-26/data/{dataset_file}"
path_to_local_home = os.environ.get("AIRFLOW_HOME", "/opt/airflow/")
parquet_file = dataset_file.replace('.csv.gz', '.parquet')
BIGQUERY_DATASET = os.environ.get("BIGQUERY_DATASET", 'airbnb_all')
...
def format_to_parquet(src_file):
    ...
    with gzip.open(src_file, 'rb') as f:
        for line in f:
            data.append(line)
    df = pd.read_csv(io.BytesIO(b''.join(data)))
    pq.write_table(pa.Table.from_pandas(df), src_file.replace('.csv.gz', '.parquet'))
...
with DAG(
    ...
    start_task >> download_dataset_task >> format_to_parquet_task >> local_to_gcs_task >> rm_task >> end_task